...
Analyzer.prototype.gather = function gather(ast, resource) {
  ...
  for (let i = 0; i < declarations.length; i++) {
    ...
    const defs = decl.defs.filter(def => this.isRequireDef(def));
    if (defs.length === 0)
      continue;
    ...
    const name = defs[0].node.init.arguments[0].value;
    const module = this.getUnresolvedModule(resource, name);
...
Analyzer.prototype.isRequireDef = function isRequireDef(def) {
